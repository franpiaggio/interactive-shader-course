...
float map(vec3 p){
    ...
    float terrace = 0.10*(0.5 + 0.5*sin(wz*1.6));   // smooth periodic tiers
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // depth fog into warm darkness (distant tracery dims but never to flat black)
    vec3 fogCol = vec3(0.035, 0.010, 0.006);
    col = mix(col, fogCol, 1.0 - exp(-tt*0.040));
