// Same terrain recap, but each layer caption also shows 1 line of example code for
// the change it applies. Self-contained: imports the shaders (read-only), reproduces
// the build-up timing, and does NOT touch Continuous.jsx (in use).
...
// caption with step + title + ONE line of example code, GSAP-animated (seeked).
const GsapCapCode = ({step, title, code, dur}) => {
  const f = useCurrentFrame();
  ...
  return (
    <div ref={root} style={{position: 'absolute', left: 64, right: 64, bottom: 70}}>
      ...
      <div className="c" style={{display: 'inline-block', marginTop: 14, fontFamily: MONO, fontSize: 27, color: '#ffd9a0',
        background: 'rgba(0,0,0,0.5)', padding: '7px 14px', borderRadius: 7, textShadow: '0 1px 4px #000'}}>{code}</div>
    </div>
  );
};
...
const cap = (from, dur, step, title, code) => (
  <Sequence key={step} from={Math.round(from * FPS)} durationInFrames={Math.round(dur * FPS)}>
    <GsapCapCode step={step} title={title} code={code} dur={dur} />
  </Sequence>
);
...
const QuadIntro = () => {
  ...
  return (
    <AbsoluteFill style={{background: '#0c0e15'}}>
      ...
      {cap(0.6, 4.0, '01 · GEOMETRY', 'We start with 2 triangles', 'gl_Position = vec4(pos, 1.0);')}
    </AbsoluteFill>
  );
};
...
const ShaderPart = () => {
  ...
  return (
    <AbsoluteFill style={{background: '#000'}}>
      ...
      {cap(0.3,  3.7, '02 · FRAGMENT',  'Every pixel gets a color',       'col = vec3(uv, 0.5);')}
      {cap(4.3,  2.6, '03 · A PLANE',   'Start with a flat ground',       'd = p.y;')}
      {cap(7.0,  3.8, '04 · NOISE',     'Paint fBM onto it',              'n = fbm(p.xz);')}
      {cap(11.0, 5.0, '05 · fBM HEIGHT','The noise becomes mountains',    'h = 16.0 * fbm(p.xz);')}
      {cap(16.5, 3.3, '06 · SUN',       'Add a light direction',          'sun = normalize(vec3(-1.,.3,.2));')}
      {cap(20.0, 3.8, '07 · LIGHT',     'It lights the slopes',           'dif = max(dot(n, sun), 0.);')}
      {cap(24.0, 4.7, '08 · MATERIALS', 'Grass, rock, snow',              'mix(rock, grass, slope);')}
      {cap(29.0, 4.7, '09 · SHADOWS',   'Long cast shadows',              'sh = shadow(p, sun);')}
      {cap(34.0, 3.7, '10 · FOG',       'Atmospheric depth',              'mix(col, sky, fog(t));')}
      {cap(37.8, 4.0, '11 · CLOUDS',    'A volumetric sky',               'cloud = fbm(p + wind);')}
